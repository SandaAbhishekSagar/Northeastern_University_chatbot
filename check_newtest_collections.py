...
def check_newtest_collections_comprehensive():
    ...
    try:
        ...
        if len(collection_details) == len(collections):
            ...
            print(f"⚠️  {len(collections) - len(collection_details)} collections failed to process")
        print()
    ...
    try:
        ...
        for i, collection in enumerate(sample_collections):
            try:
                ...
                print(f"  ✅ Collection {i+1}: {collection.name} - metadata accessible")
            except Exception as e:
                print(f"  ❌ Collection {i+1}: {collection.name} - metadata error: {e}")
        ...
        print(f"❌ Error during retrieval limit tests: {e}")
...
def main():
    ...
    if results:
        ...
        if results['success_rate'] == 100:
            ...
            print("⚠️  Some collections failed to process - check for corrupted data")
