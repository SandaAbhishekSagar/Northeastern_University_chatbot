#!/usr/bin/env python3
"""
Ultra-optimized migration to Chroma Cloud with very strict size limits
"""
...
def chunk_text_ultra(text: str, max_size: int = 8000) -> List[str]:
...
def clean_metadata_ultra(metadata: Dict[str, Any]) -> Dict[str, Any]:
    """Ultra-clean metadata by keeping only the most essential fields"""
    if not metadata:
        return {}
    
    # Keep only the most essential metadata fields
    essential_fields = [
        'id', 'title', 'source_url', 'university_id', 
...
def migrate_collection_ultra_optimized(collection_name: str, local_path: str = "./chroma_data"):
    """Migrate a collection with ultra-optimized chunking and metadata cleanup"""
    
    print(f"🚀 Starting ultra-optimized migration of {collection_name}...")
    ...
    # Process documents and create ultra-optimized chunks
    optimized_ids = []
    optimized_documents = []
    optimized_metadatas = []
    ...
    for i, (doc_id, document, metadata, embedding) in enumerate(zip(
        ...
        # Ultra-clean metadata
        clean_meta = clean_metadata_ultra(metadata)
        
        # Calculate size with ultra-cleaned metadata
        ...
        if total_size > 12000:  # Very conservative limit
            large_docs_count += 1
            print(f"📝 Ultra-chunking large document {doc_id} ({total_size} bytes)")
            
            chunks = chunk_text_ultra(document)
    ...
    print(f"✂️ Ultra-chunked {large_docs_count} large documents into {chunked_docs_count} chunks")
    print(f"📤 Total documents after ultra-optimization: {len(optimized_ids)}")
    
    # Save ultra-optimized data to a new local collection
    optimized_collection_name = f"{collection_name}_ultra_optimized"
    ...
    print(f"✅ Created ultra-optimized collection: {optimized_collection_name}")
    
    # Test a larger sample to verify sizes
    test_sample = optimized_collection.get(limit=100, include=['documents', 'metadatas'])
    test_sizes = []
    ...
    if test_sizes:
        ...
        print(f"📊 Test sample (100 docs) - Average: {avg_size:.0f} bytes, Max: {max_size} bytes, Min: {min_size} bytes")
...
if __name__ == "__main__":
    print("🔧 Starting ultra-optimized migration to Chroma Cloud...")
    print("=" * 60)
    
    # Process the documents collection
    optimized_name = migrate_collection_ultra_optimized("documents")
